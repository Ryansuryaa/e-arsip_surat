...
class LetterController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->file('letter_file')){
            $validatedData['letter_file'] = $request->file('letter_file')->store('assets/letter-file', 'public');
        }
    ...
    {
        if (request()->ajax()) {
            $query = Letter::with(['department'])->where('letter_type', 'Surat Masuk')->latest()->get();

            return Datatables::of($query)
                ->addColumn('sender', function($item) {
                    // GET NAME OF SENDER
                    $senderDiLetter = $item->sender;
                    if(is_numeric($senderDiLetter) && preg_match('/^\d+$/', $senderDiLetter)) {
                        $nameDiSender = Sender::where('id', $senderDiLetter)->first();
                        return $nameDiSender->name;
                    } else {
                        return $senderDiLetter;
                    }
                })
    ...
    {
        if (request()->ajax()) {
            $query = Letter::with(['department'])->where('letter_type', 'Surat Keluar')->latest()->get();
    ...
    public function show($id)
    {
        ...
        if(is_numeric($senderDiLetter) && preg_match('/^\d+$/', $senderDiLetter)) {
            $nameDiSender = Sender::where('id', $senderDiLetter)->first();
            $namaSender = $nameDiSender->name;
        } else {
            $namaSender = $senderDiLetter;
        }

        return view('pages.admin.letter.show', compact('item', 'namaSender'));
    }
    ...
    public function download_letter($id)
    {
        ...
        return Storage::response($item->letter_file);
    }
